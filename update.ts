...
/**
 * For scoped packages (example: @types/react) we should
 * replace slash with %2f, so this function normalizes name
 * of such packages.
 *
...
const check = dependencies => {
  return Object.keys(dependencies).map(dependency => {
    ...
    return new Promise(resolve => {
      request(`https://registry.npmjs.com/${normalizeDependencyName(dependency)}`, (error, response, body) => {
